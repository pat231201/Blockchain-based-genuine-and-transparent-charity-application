...
contract SmaartContract {
    ...
    constructor() {
        ...
        addressVerifierMap[
            " 0xBf1a8D3c3Bf9D2f38215287580895378Ee03a723"
            //0x27510d27b0B5c8c813A893726DcEAB6a933345da
        ] = "PAN";
        verifierAddressMap[
            "PAN"
        ] = "0xBf1a8D3c3Bf9D2f38215287580895378Ee03a723";
        //0x27510d27b0B5c8c813A893726DcEAB6a933345da
        // addressVerifierMap[
        //     "0xFc29C015DC4BA8cfBC4E8fd2F0749a598365bf11"
        //     //0xf4b949991740de4c090851059ebf9d985fb46bb3
        // ] = "MNNIT A";
        // verifierAddressMap[
        //     "MNNIT A"
        // ] = "0xFc29C015DC4BA8cfBC4E8fd2F0749a598365bf11";
        // //0xf4b949991740de4c090851059ebf9d985fb46bb3
        addressVerifierMap[
            "0xcBb12b49EBBb64DB44D1ac2dFF5d8CC5F68D3212"
            //0xb9f72ef4c0f0d306a6b12e3d869be1c8a126f0ae
        ] = "UIDAI";
        verifierAddressMap[
            "UIDAI"
        ] = "0xcBb12b49EBBb64DB44D1ac2dFF5d8CC5F68D3212";
    ...
    function addVReq(
        ...
        VerifierVReqs[verifier].push(
            VReq(
                ...
                0,
                UserVReqs[msg.sender].length
            )
        );
        UserVReqs[msg.sender].push(VReqMeta(verifier, 0));
    }

    function showVerifierVerificationReqListLength()
        public
        view
        returns (uint)
    {
    ...
    function showVerifierVerificationReqList(
        uint index
    )
        public
        view
        returns (string memory cid, address user, int status, uint metaIndex)
    {
        return (
    ...
    function showVerifierVerificationReqScopeList(
        uint index
    )
        public
        view
    ...
    function showVerifierVerificationReqScopeBoolsList(
        uint index
    ) public view returns (int isOver18, int isCollegeStudent) {
        return (
    ...
    function showUserVerificationReqList(
        uint index
    ) public view returns (address verifier, int status) {
        return (
    ...
    function verifyReq(address user, uint index, bool decision) public {
        if (decision) {
            ...
            if (
                keccak256(abi.encodePacked(vreq.scopes.name)) !=
                keccak256(abi.encodePacked(""))
            ) {
                UserInfo[vreq.user].name = vreq.scopes.name;
            }
            if (
                keccak256(abi.encodePacked(vreq.scopes.sex)) !=
                keccak256(abi.encodePacked(""))
            ) {
                UserInfo[vreq.user].sex = vreq.scopes.sex;
            }
            if (
                keccak256(abi.encodePacked(vreq.scopes.dob)) !=
                keccak256(abi.encodePacked(""))
            ) {
            ...
            if (
                keccak256(abi.encodePacked(vreq.scopes.email)) !=
                keccak256(abi.encodePacked(""))
            ) {
                UserInfo[vreq.user].email = vreq.scopes.email;
            }
            if (
                keccak256(abi.encodePacked(vreq.scopes.college)) !=
                keccak256(abi.encodePacked(""))
            ) {
            ...
            if (vreq.scopes.isCollegeStudent != 0) {
                UserInfo[vreq.user].isCollegeStudent = vreq
                    .scopes
                    .isCollegeStudent;
            }
        } else {
    ...
    function giveAccess(
        ...
        if (college) {
            UserOrgAccess[msg.sender][org].college = UserInfo[msg.sender]
                .college;
        } else {
            UserOrgAccess[msg.sender][org].college = "";
        }
        if (isOver18) {
            UserOrgAccess[msg.sender][org].isOver18 = UserInfo[msg.sender]
                .isOver18;
        } else {
            UserOrgAccess[msg.sender][org].isOver18 = 0;
        }
        if (isCollegeStudent) {
            UserOrgAccess[msg.sender][org].isCollegeStudent = UserInfo[
                msg.sender
            ].isCollegeStudent;
        } else {
    ...
    function showUserInfoBoolsByOrg(
        address user
    ) public view returns (int isOver18, int isCollegeStudent) {
        return (
    ...
    ) public view returns (string memory verifierAddress) {
        return verifierAddressMap[verifier];
    }

    function getVerifierName(
        string memory verifierAddress
    ) public view returns (string memory verifierName) {
