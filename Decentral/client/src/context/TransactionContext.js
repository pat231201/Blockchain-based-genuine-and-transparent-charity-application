...
export const TransactionsProvider = ({ children }) => {
  ...
  const submitDocument = async (
    ...
    try {
      if (ethereum) {
        ...
        if (
          dob !== "" &&
          new Date(dob).getFullYear() <= new Date().getFullYear() - 18
        )
          isOver18 = 1;
        const isCollegeStudent = college === "" ? 0 : 1;
        const res = await transactionsContract.addVReq(
          verifier,
          cid,
          name,
          sex,
  ...
  const loadUserList = async () => {
    try {
      if (ethereum) {
        ...
        const userList = [];
        const length =
          await transactionsContract.showUserVerificationReqListLength();
        console.log(length);
        let i = 0;
        while (i < length) {
          const obj = {};
  ...
  const loadVerifierList = async () => {
    try {
      if (ethereum) {
        ...
        const userList = [];
        const length =
          await transactionsContract.showVerifierVerificationReqListLength();
        let i = 0;
        while (i < length) {
          const obj = {};
          let res = await transactionsContract.showVerifierVerificationReqList(
            i
          );
          obj.cid = res.cid;
          ...
          res = await transactionsContract.showVerifierVerificationReqScopeList(
            i
          );
          obj.sex = res.sex;
          obj.name = res.name;
          ...
          res =
            await transactionsContract.showVerifierVerificationReqScopeBoolsList(
              i
            );
  ...
  const verify = async (user, metaIndex, decision) => {
    try {
      if (ethereum) {
        const transactionsContract = createEthereumContract();

        await transactionsContract.verifyReq(user, metaIndex, decision);
      } else {
        console.log("Ethereum is not present");
      }
    } catch (error) {
